...
> **Skill specifica per il progetto EYS/Retinitis Pigmentosa.**
> Usa il namespace `retina:` e include relazioni EYS-specifiche (`transportRegulates`, `regulatesLocalizationOf`).
> La sezione finale aggiunge le varianti genetiche di EYS dal PDF del progetto.
> Per generare RDF generico su qualsiasi gene usa `/rdf-turtle GENE`.

Genera un file RDF in formato Turtle (.ttl) per un gene retinico, combinando dati da UniProt, STRING e dal PDF di riferimento del progetto (studio RP25/EYS).

## Come usare
`/eys-rdf-turtle GENE_SYMBOL`

Esempio: `/eys-rdf-turtle EYS` oppure `/eys-rdf-turtle AIPL1`
